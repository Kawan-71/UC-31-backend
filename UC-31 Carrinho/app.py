...
@app.route('/visitas')
def visitas():
    contador = session.get('visitas', 0)
    return f'Você visitou este site {contador} vezes.'
...
@app.route('/favoritar/<produto>')
def favoritar(produto):
    favoritos = session.get('favoritos', [])

    if produto not in favoritos:
...
def ver_favoritos():
    favoritos = session.get('favoritos', [])
    return f"Seus favoritos: {favoritos}"
